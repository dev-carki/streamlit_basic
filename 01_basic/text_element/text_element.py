...
def show():
    index_num = 1
    desc('title - 제목', index_num)
    st.title('제목')
    index_num += 1
    
    desc('header - 헤더 / 마크다운', index_num)
    st.header('헤더')
    index_num += 1
    
    desc('subheader - 서브 헤더 (헤더보다 작음) / 마크다운', index_num)
    st.subheader('서브헤더')
    index_num += 1
    
    desc('markdown - 마크다운 양식 적용 / 크기 조절도 가능해서 위 속성들보다 일반적으로 사용됨', index_num)
    st.markdown('마크다운')
    st.markdown("""
                마크다운(띄워쓰기 네칸 = 줄바꿈)    
                줄바꿈
    ...
    desc('latex - 수학 표현식', index_num)
    st.latex(r"""result = \frac{a}{b}""")
    index_num += 1
    
    desc('html - html 문법', index_num)
    st.html("""
